...
public class TransactionService {
    ...
    public TransactionResponse getById(Long id) {
        Transaction existingTransaction = this.transactionRepository.findById(id).orElseThrow(()-> new ApiException("Transaction not found"));
    ...
    public String save(TransactionRequest transactionRequest, String token) {
        ...
        Category category = this.categoryRepository.findById(transactionRequest.getCategoryId()).orElseThrow(()->new ApiException("Category not found"));
    ...
    public String update(Long id, TransactionRequest transactionRequest, String token) {
        Long userId = jwtService.extractUserId(token);
        Transaction foundTransaction = this.transactionRepository.findByIdAndUserId(id,userId).orElseThrow(()->new ApiException("Transaction not found"));
